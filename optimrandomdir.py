...
def optimrandomdir(objfunction, coeffs, args=(), bounds=(), coeffdeviation=1.0, iterations=1000, startingscale=2, endscale=0.0):
   ...
   #while (np.linalg.norm(coeffdeviation)>1e-6):
      ...
      if X1<X0:  #New is better
         ...
         #coeffdeviation += coeffset*(-0.1*coeffdeviation-0.1*(abs(coeffvariation)/np.linalg.norm(coeffvariation)*setfrac)) #update normalized deviation vector by reducing by previous finished direction, correct mean accorting to the non-zero fraction, seems to be indeed better than "+abs(coeffvariation)"
         #coeffdeviation=coeffdeviation/np.linalg.norm(coeffdeviation)
